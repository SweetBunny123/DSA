...
Node* Search_Equal(Node* root, int X){
    Node* travel = New_Node();
    travel = root;
    while(travel != nullptr){
        if(travel->key == X){
...
int main(){
    ...
    cout << "Input the range between which you want to resolve the more than average query: ";
    cin >> X >> Y;
    int range_sum = Prefix_Sum(root, Y) - Prefix_Sum(root, X);
    int range_count = (Find_Rank(root, X) + 1) - (Find_Rank(root, Y) + 1);
    if(Search_Equal(root, X) != nullptr){
        int repetition = Search_Equal(root, X)->repeat;
        range_sum -= X*repetition;
    }
    if(Search_Equal(root, Y) != nullptr){
        int repetition = Search_Equal(root, Y)->repeat;
        range_count -= repetition;
    }
    cout << range_sum << " " << range_count << endl;
    int average = range_sum/range_count;
    range_count = (Find_Rank(root, average) + 1) - (Find_Rank(root, Y) + 1);
    if(Search_Equal(root, Y) != nullptr){
        int repetition = Search_Equal(root, Y)->repeat;
        range_count -= repetition;
    }
    cout << "The number of elements between " << average << " and " << Y << " = " << range_count << endl;
    ...
    range_count = (Find_Rank(root, X) + 1) - (Find_Rank(root, Y) + 1);
    if(Search_Equal(root, X) != nullptr){
        int repetition = Search_Equal(root, X)->repeat;
        range_sum -= X*repetition;
    }
