...
struct bintree {
    long long data; // Changed from int to long long
    struct bintree* left;
    struct bintree* right;
    bintree(long long val) { // Changed from int to long long
        data = val;
        left = right = NULL;
    }
}; 
...
void levelOrderTraversal(struct bintree* node) {
    ...
    while (!q.empty()) {
        ...
        q.pop();
        if (curr->left) {
            curr->left->data = curr->left->data + curr->data;
            q.push(curr->left);
        }
        if (curr->right) {
            curr->right->data = curr->right->data + curr->data;
            q.push(curr->right);
        }
    }
}
